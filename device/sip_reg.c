...
//1414
int set_x_header_status(pjsip_regc *regc) {
	struct pjsip_generic_string_hdr *CustomHeader = malloc(sizeof(pjsip_generic_string_hdr));
	pj_str_t name = pj_str("X-status-1414");
	pj_str_t value = pj_str("8");
	pjsip_generic_string_hdr_init2(CustomHeader, &name, &value);
	pj_list_push_back(&regc->hdr_list, CustomHeader);
	return 0;
}
//1414
