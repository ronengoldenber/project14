...
static int isNumber(char* number) {
	...
	len = strnlen(number, 15);
	for(i = 0; i < len; i++) {
		if(number[i] < '0' || number[i] > '9') {
			PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed must be a number [%s]", number));
			return 0;
		}
	}
	return 1;
}
static int isUSCallAllowed(char* number) {
	int len = 0;
	char area_code[3];
	if (!number) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed number is empty"));
		return 0;
	}
	len = strnlen(number, 11);
	if(len != 11) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed because the number is not 11 digits US number is allowed [%s]", number));
		return 0;
	}
	if(!isNumber(number)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed must be a number [%s]", number));
	...
	area_code[0] = number[1]; area_code[1] = number[2]; area_code[2] = number[3];
	if(	strncmp(area_code, "650", 3) && strncmp(area_code, "925", 3) && strncmp(area_code, "800", 3) && 
		strncmp(area_code, "212", 3) && strncmp(area_code, "718", 3) && strncmp(area_code, "408", 3) && 
		strncmp(area_code, "631", 3) && strncmp(area_code, "215", 3) && strncmp(area_code, "718", 3) &&
		strncmp(area_code, "646", 3) && strncmp(area_code, "917", 3) && strncmp(area_code, "201", 3)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed to this area code [%s]", area_code));
		return 0;
	}
	return 1;
}
static int isILShortDial(char* number) {
	int len = 0;
	if(!number) {
		PJ_LOG(3,(THIS_FILE, "1414: The number is empty while checking IL short dial "));
		return 0;
        }
	len = strnlen(number, 6);
	if(len < 2 || len > 5) {
		PJ_LOG(3,(THIS_FILE, "1414: The number [%s], is too short [%d]  for IL short dial ", number, len));
		return 0;
	}
	if(!isNumber(number+1)) {
		PJ_LOG(3,(THIS_FILE, "1414: The number [%s] is not a short dial number ", number ,len));
		return 0; 
	}
	if(number[0] != '*') {
		PJ_LOG(3,(THIS_FILE, "1414: The number [%s] is not a short dial number because there is no * in the beginning ", number ,len));
		return 0;
	}
	return 1;
}
static int isILInternational(char* number) {
	int len = 0;
	if(!number) {
		PJ_LOG(3,(THIS_FILE, "1414: The number is empty while checking IL international calls "));
		return 1;
	}
	len = strnlen(number, 11);
	if(len > 10) {
		PJ_LOG(3,(THIS_FILE, "1414: The number [%s] cannot be more than 10 digits while checking IL international calls ", number));
		return 1;
	}
	if(	!strncmp(number, "00", 2) || !strncmp(number, "0110", 4) || !strncmp(number, "012", 3) || !strncmp(number, "013", 3) || !strncmp(number, "014", 3) ||
		!strncmp(number, "015", 3) || !strncmp(number, "016", 3) || !strncmp(number, "017", 3) || !strncmp(number, "018", 3) || !strncmp(number, "019", 3)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed must be an IL number [%s] international number", number));
		return 1;
        }
	return 0;
}
static int isILLandline(char* number) {
	int len = 0;
	if(!number) {
		PJ_LOG(3,(THIS_FILE, "1414: The number is empty while checking IL international calls "));
		return 1;
	}
	len = strnlen(number, 10);
	if(len != 9) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed because the number is not 9 digits IL landline number [%s] ", number));
		return 0;
	}
	if(!strncmp(number, "02", 2) || !strncmp(number, "03", 2) || !strncmp(number, "04", 2) || !strncmp(number, "08", 2) || !strncmp(number, "09", 2)) {
		if(number[2] >= '5' && number[2] <= '9') {
			PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to Bezeq number [%s]", number));
			return 1;
		}
		if(number[2] == '3' && (number[3] == '0' || number[3] == '1' || number[3] == '2')) {
			PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to hot landline number [%s]", number));
			return 1;
		}
		if(!strncmp(number + 2, "372", 3) || !strncmp(number + 2, "374", 3)) {
			PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to partner landline number [%s]", number));
			return 1;
		}
		if(!strncmp(number + 2, "373", 3)) {
			PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to cellcom landline number [%s]", number));
			return 1;
		}
	}
	PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed to IL number [%s] 9 digits", number));
	return 0;
}
static int isILMobile(char* number) {
	int len = 0;
	if(!number) {
		PJ_LOG(3,(THIS_FILE, "1414: The number is empty while checking IL international calls "));
		return 1;
	...
	len = strnlen(number, 10);
	if(len != 10) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed because the number is not 10 digits IL mobile number [%s] ", number));
		return 0;
	}
	if(!strncmp(number, "0722", 4) || !strncmp(number, "0723", 4) || !strncmp(number, "0747", 4)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to Partner mobile number [%s]", number));
		return 1;
	}
	if(!strncmp(number, "0732", 4) || !strncmp(number, "0733", 4)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to Cellcom mobile number [%s]", number));
		return 1;
	}
	if(!strncmp(number, "0768", 4)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to Bezeq7 number [%s]", number));
		return 1;
	}
	if(!strncmp(number, "077", 3)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to Hot7 number [%s]", number));
		return 1;
	}
	if(!strncmp(number, "0782", 4)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to Golan number [%s]", number));
		return 1;
	}
	if(!strncmp(number, "050", 3)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to Pelephone number [%s]", number));
		return 1;
	}
	if(!strncmp(number, "052", 3)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to Cellcom number [%s]", number));
		return 1;
	}
	if(!strncmp(number, "053", 3) || !strncmp(number, "057", 3)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to HOT number [%s]", number));
		return 1;
	}
	if(!strncmp(number, "054", 3)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to Partner number [%s]", number));
		return 1;
	}
	if(!strncmp(number, "058", 3)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to Golan number [%s]", number));
		return 1;
	}
	if(!strncmp(number, "05566", 5) || !strncmp(number, "05567", 5)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to Rami Levi number [%s]", number));
		return 1;
	}
	if(!strncmp(number, "05587", 5) || !strncmp(number, "05588", 5)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to You Phone number [%s]", number));
		return 1;
	}
}
static int isILTollFree(char* number) { 
	int len = 0;
	if(!number) {
		PJ_LOG(3,(THIS_FILE, "1414: The number is empty while checking IL toll free calls "));
		return 1;
	}
	len = strnlen(number, 10);
	if(len != 10) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed because the number is not 10 digits toll free IL number [%s] ", number));
		return 0;
	}
	if(!strncmp(number, "1800", 4) || !strncmp(number, "1700", 4)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed legal toll free number [%s] ", number));
		return 1;
	}
	PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed because the number is not toll free IL number [%s] ", number));
	return 0;
}
static int isILCallAllowed(char* number) {
	int len = 0;
	int i = 0;
	if(!number) { 
		PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed to IL number is empty"));
		return 0;
	}
	if(isILShortDial(number)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call allowed short dial [%s]", number));
		return 1;
	}
	len = strnlen(number, 10);
	if(len != 9 && len != 10) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed because the number is not 9 or 10 digits US number is allowed [%s]", number));
		return 0;
	}
	if(!isNumber(number)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed must be an IL number [%s]", number));
		return 0;
	}
	// block international in any case
	if (isILInternational(number)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed must be an IL number [%s] international number", number));
		return 0;
	}
	// local landline number
	if(len == 9 && isILLandline(number)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to IL local number [%s]", number));
		return 1;
	}
	// mobile numbers
	if(len == 10 && isILMobile(number)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to IL mobile number [%s]", number));
		return 1;
	}
	// toll free numbers 
	if(len == 10 && isILTollFree(number)) {
		PJ_LOG(3,(THIS_FILE, "1414: Call is allowed to IL toll free number [%s] ", number));
		return 1;
	}
	PJ_LOG(3,(THIS_FILE, "1414: Call is not allowed to this number [%s]", number));
	return 0;
}
static int make_call(pjsua_call_id call_id, pjsip_rx_data *rdata) {
	...
	int response = 0;
	int i = 0, j = 0;
	name = pj_str("X-call-number");
	if (!(hdr = pjsip_msg_find_hdr_by_name(rdata->msg_info.msg, &name, NULL))) {
		PJ_LOG(3, (THIS_FILE, "1414: Cannot allow call call number not found "));
		return 0;
	}
	PJ_LOG(3, (THIS_FILE, "1414: Found header name '%.*s' value '%.*s'", hdr->name.slen, hdr->name.ptr, hdr->hvalue.slen, hdr->hvalue.ptr));
	memset(number, 0, 32);
	memset(cmd, 0, 256);
	for(i = 0; i < hdr->hvalue.slen && i < 32; i++) {
		if(hdr->hvalue.ptr[i] >= '0' && hdr->hvalue.ptr[i] <= '9') {
			number[j++] = hdr->hvalue.ptr[i];
		}
	}
	if(!isUSCallAllowed(number)) {
//	if(!isILCallAllowed(number)) {
		PJ_LOG(3, (THIS_FILE, "1414: The call is not allowed to this number [%s]", number));
		return 0;
	}
	snprintf(cmd, 256, "/home/pi/on_incoming_call.sh %s %d", number, call_id);
	PJ_LOG(3, (THIS_FILE, "running command %s", cmd));
	response = system(cmd);
	return !response;
}
static int call_hangup1414(pjsua_call_id call_id) {
	char cmd[256];
	memset(cmd, 0, 256);
	snprintf(cmd, 256, "/home/pi/on_call_hangup.sh %d", call_id);
	PJ_LOG(3, (THIS_FILE, "Call hangup call [%s]", cmd));
	int response = system(cmd);
	return !response;
}
static void send_dtmf(int dtmf) {
	char* cmd = NULL;
	PJ_LOG(3, (THIS_FILE, "Got DTMF [%c]", dtmf));
	switch (dtmf) {
		case '0': cmd = "sudo /home/pi/ofono/test/set-dtmf0"; break;
		case '1': cmd = "sudo /home/pi/ofono/test/set-dtmf1"; break;
		case '2': cmd = "sudo /home/pi/ofono/test/set-dtmf2"; break;
		case '3': cmd = "sudo /home/pi/ofono/test/set-dtmf3"; break;
		case '4': cmd = "sudo /home/pi/ofono/test/set-dtmf4"; break;
		case '5': cmd = "sudo /home/pi/ofono/test/set-dtmf5"; break;
		case '6': cmd = "sudo /home/pi/ofono/test/set-dtmf6"; break;
		case '7': cmd = "sudo /home/pi/ofono/test/set-dtmf7"; break;
		case '8': cmd = "sudo /home/pi/ofono/test/set-dtmf8"; break;
		case '9': cmd = "sudo /home/pi/ofono/test/set-dtmf9"; break;
		default : return;
	}
	PJ_LOG(3, (THIS_FILE, "Sending dtmf [%s]", cmd));
	int response = system(cmd);
}
//1414
static void ringback_start(pjsua_call_id call_id)
...
//1414
	if(call_info.last_status != 410) {
		call_hangup1414(call_id);
	}
//1414
...
//    ring_start(call_id);
//1414
	call_response = make_call(call_id, rdata);
	if(!call_response) {
		pjsua_call_hangup(call_id, 410, NULL, NULL);
	}
//1414    
...
static pj_status_t app_destroy()
{
    ...
    int cli_fe = 0;
//1414
    pj_uint16_t cli_telnet_port = 1414;
//1414
#ifdef STEREO_DEMO
    if (app_config.snd) {
...
//1414	cli_telnet_port = app_config.cli_cfg.telnet_cfg.port;	
	cli_telnet_port = 1414;
//1414
    }
...
//1414	app_config.cli_cfg.telnet_cfg.port = cli_telnet_port;
	app_config.cli_cfg.telnet_cfg.port = 1414;
//1414
    }
