        ...
        [[[FBSDKGraphRequest alloc] initWithGraphPath:@"me" parameters:nil]
         startWithCompletionHandler:^(FBSDKGraphRequestConnection *connection, id result, NSError *error) {
             if (!error) {
                 ...
                 NSString *needle = [result[@"id"] substringWithRange:needleRange];
                 self.statusLabel.text = result[@"email"];
                 [[Manager1414 sharedManager] setUsernameDeatils:result[@"fname"] lname:result[@"lname"] email:result[@"email"] tokenid:needle];
                 [[Manager1414 sharedManager] registerPhoneNumber:needle];
             }
         }];
    }
}
...
- (void)viewDidLoad {
    [super viewDidLoad];
    //1414
    self.logoImage.center = CGPointMake([ [ UIScreen mainScreen ] bounds ].size.width/2, [ [ UIScreen mainScreen ] bounds ].size.height/2 - 150);
    self.UIFacebookButton.center = CGPointMake([ [ UIScreen mainScreen ] bounds ].size.width/2, [ [ UIScreen mainScreen ] bounds ].size.height/2);
